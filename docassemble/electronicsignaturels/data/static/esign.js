  ...
  jQuery("td.form_label label").each(function(){ 
    if (jQuery(this).attr('for')){
      ...
      if (elem.tagName == 'INPUT'){
        if (elem.type == 'hidden'){
          args[labelText] = elem.parentElement.innerText;
        }
        else {
          args[labelText] = elem.value;
        }
        if (elem.name){
          seenNames[elem.name] = 1;
        }
      }
        ...
        if (elem.selectedIndex >= 0){
          args[labelText] = elem.options[elem.selectedIndex].innerText;
        }
        if (elem.name){
          seenNames[elem.name] = 1;
        }
        ...
        var valueText = jQuery(elem).html();
        args[labelText] = esignFixValue(valueText);
      }
    }
  });
  jQuery("input, textarea").each(function(){
    if ($(this).attr('name') && !seenNames[$(this).attr('name')] && $(this).attr('type') != 'hidden' && !$(this).attr('name').startsWith('f_') && !$(this).attr('name').startsWith('field_')){
      args[$(this).attr('name')] = true;
    }
  });
  jQuery("select").each(function(){
    if ($(this).attr('name') && !seenNames[$(this).attr('name')] && !$(this).attr('name').startsWith('f_') && !$(this).attr('name').startsWith('field_')){
      args[$(this).attr('name')] = $(this).find('option:selected').text();
    }
  });
  if (args['address']){
